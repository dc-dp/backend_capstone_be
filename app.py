...
def create_all():
    with app.app_context():
        ...
        org_data = (
            ...
            .filter(Organizations.name == "DevPipeline")
            .first()
        )
        if org_data == None:
            print(
                "DevPipeline organization not found. Creating DevPipeline Organization in database..."
            )
        ...
        user_data = (
            db.session.query(AppUsers).filter(AppUsers.email == config.su_email).first()
        )
        if user_data == None:
            print(
                f"Super Admin not found! Creating Super Admin user ({config.su_email})..."
            )
...
def create_app(config_file=None):
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = os.getenv(
        "DATABASE_URI", f"postgres://{database_host}/{database_name}"
    )
    ...
    if app.debug:
        ...
        if app.env == "test":
            ...
            raise EnvironmentError(
                "Invalid environment for app state. (Look inside __init__.py for help)"
            )
    else:
